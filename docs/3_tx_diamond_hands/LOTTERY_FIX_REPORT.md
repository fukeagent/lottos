...
* **After:** We increased it to `MAX_WINNER_ATTEMPTS = 30`. Since we use a cached snapshot system and pull payments, iterating 30 times consumes very little gas and minimizes the chance of round rollover due to empty Fenwick trees or ghost wallets, guaranteeing a winner in almost all draws.
