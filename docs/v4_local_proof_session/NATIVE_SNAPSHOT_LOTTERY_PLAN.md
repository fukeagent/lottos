...
3. **Append-Only Buckets**: Change the bucket logic to never "Swap and Pop" users. We just push them.
4. **Trigger Commitment**:
   ...
   function triggerLottery() external {
       ...
       // Snapshot the 15 active bucket totals into storage (O(1) gas)
       // ...
   }
   ```
5. **Winner Selection (3 Fallbacks)**:
   ...
   function selectWinner() external {
       ...
       for (uint256 i = 0; i < 3; i++) {
           ...
           uint8 winningBucket = _selectBucket(seed, r.roundId);
           uint256 candidateIndex = seed % bucketHolders[winningBucket].length;
           ...
           bool isGhost = msb(snapshotBalance) != winningBucket;
